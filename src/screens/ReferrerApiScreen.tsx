...
const ReferrerApiScreen: React.FC = () => {
  const navigation = useNavigation();
  const [referrer, setReferrer] = useState<string>('');

  const fetchReferrer = async () => {
    try {
      const result = await TrackingSDK.fetchInstallReferrer();
      setReferrer(result);
      Alert.alert('Fetched Referrer', result);
    } catch (error) {
...
export default ReferrerApiScreen;
